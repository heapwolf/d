...
api.updateConfig = function updateConfig(callback) {

  fs.writeFileSync(__dirname + '/../config/config.json', JSON.stringify(config, true, 2));
  log(0, 'Updated the local configuration file');
};
...
function softwareUpdate(callback) {
  ...
  request(
    {
      uri: 'http://registry.npmjs.org/d/latest',
      timeout: 400
    }, 
    function (err, res, body) {
      ...
      try {
        ...
        if (semver.gt(pkg.version, pkg.version)) {
          ...
          log(1, 'A newer version of this program is available.');
          process.exit(0);
