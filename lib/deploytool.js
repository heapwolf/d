...
git.exec = function(args, opts, callback) {
  ...
  cp.on('close', function (code) {
    ...
    if (exitcode > 0 && stderr.length > 0) {
      ...
      log(1, 'The host at `' + oldhost.address + '` on port `' + oldhost.port + 
        '` is unresponsive.');
      ...
      callback(common.join(stderr), common.join(stdout));
    }
  });
  ...
  fs.stat(process.env['PWD'] + '/.git', function(err, stat) {
    ...
    if (err) {
      ...
      git.exec(['init'], {}, function(err, data) {
        ...
        var root = api.config.protocol + '://' + api.config.hosts[api.config.index];
        var repo = git.getUserIdentity().name + '/' + pkg.name;
        var args = ['remote', 'add', 'origin-' + api.config.index, root + '/' + repo + '.git'];
...
//
git.formulateOrigin = function formulateOrigin(config) {
